package com.fw.zycoder.errorpage;
...
import com.fw.zycoder.errorpage.activity.DefaultErrorActivity;
...
public final class CustomActivityOnCrash {
  ...
  private static final String EXTRA_RESTART_ACTIVITY_CLASS =
      "com.fw.zycoder.errorpage.EXTRA_RESTART_ACTIVITY_CLASS";
  private static final String EXTRA_SHOW_ERROR_DETAILS =
      "com.fw.zycoder.errorpage.EXTRA_SHOW_ERROR_DETAILS";
  private static final String EXTRA_STACK_TRACE = "com.fw.zycoder.errorpage.EXTRA_STACK_TRACE";
  private static final String EXTRA_IMAGE_DRAWABLE_ID =
      "com.fw.zycoder.errorpage.EXTRA_IMAGE_DRAWABLE_ID";
  private static final String EXTRA_EVENT_LISTENER =
      "com.fw.zycoder.errorpage.EXTRA_EVENT_LISTENER";

  // General constants
  private static final String INTENT_ACTION_ERROR_ACTIVITY = "com.fw.zycoder.errorpage.ERROR";
  private static final String INTENT_ACTION_RESTART_ACTIVITY = "com.fw.zycoder.errorpage.RESTART";
  private static final String CAOC_HANDLER_PACKAGE_NAME = "com.fw.zycoder.errorpage";
              ...
              if (hasCrashedInTheLastSeconds(application)) {
                ...
                if (isStackTraceLikelyConflictive(throwable, errorActivityClass)) {
                  ...
                  if (stackTraceString.length() > MAX_STACK_TRACE_SIZE) {
                    ...
                    stackTraceString =
                        stackTraceString.substring(0, MAX_STACK_TRACE_SIZE - disclaimer.length())
                            + disclaimer;
                  }
  ...
  public static String getAllErrorDetailsFromIntent(Context context, Intent intent) {
    ...
    // Ideally, we should not use this method at all... It is only formatted this way because of
    // coupling with the default error activity.
  ...
  public static Class<? extends Activity> getRestartActivityClassFromIntent(Intent intent) {
    Serializable serializedClass =
        intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_RESTART_ACTIVITY_CLASS);
  ...
  public static EventListener getEventListenerFromIntent(Intent intent) {
    Serializable serializedClass =
        intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_EVENT_LISTENER);
  ...
  /**
   * INTERNAL method that returns the build date of the current APK as a string, or null if unable
   * to determine it.
   *
   * @param context A valid context. Must not be null.
  ...
  /**
   * INTERNAL method used to guess which activity must be called from the error activity to restart
   * the app.
   * It will first get activities from the AndroidManifest with intent filter
   * <action android:name="com.fw.zycoder.errorpage.RESTART" />,
  ...
  private static Class<? extends Activity> getRestartActivityClassWithIntentFilter(
    ...
    Intent searchedIntent =
        new Intent().setAction(INTENT_ACTION_RESTART_ACTIVITY).setPackage(context.getPackageName());
    List<ResolveInfo> resolveInfos =
        context.getPackageManager().queryIntentActivities(searchedIntent,
   ...
   * It will first get activities from the AndroidManifest with intent filter
   * <action android:name="com.fw.zycoder.errorpage.ERROR" />,
