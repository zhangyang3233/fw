...
public abstract class GsonRequestBuilder<T> implements VolleyRequestBuilder {
  ...
  public GsonRequestBuilder setInterceptor(Interceptor<T> interceptor) {
  ...
  public GsonRequestBuilder setNeedCache(boolean needCache) {
    this.needCache = needCache;
    return this;
  }

  public GsonRequestBuilder setIsNeedFakeHeader(boolean isNeedFakeHeader) {
    this.isNeedFakeHeader = isNeedFakeHeader;
    return this;
  }

  public GsonRequestBuilder setIsNeedToastError(boolean isNeedToastError) {
    this.needToastError = isNeedToastError;
    return this;
  }

  public GsonRequestBuilder setCacheTime(long cacheTime) {
    this.cacheTime = cacheTime;
    return this;
  }

  public GsonRequestBuilder setInvalidateCache(boolean needInvalidateCache) {
    this.needInvalidateCache = needInvalidateCache;
    return this;
  }

  public GsonRequestBuilder setLogEnable(boolean isLogEnable) {
  ...
  public ApiRequest<T> build() {
     ...
     */
    if (mCallback != null) {
      request = new GsonRequest<T>(mMethod, requestUrl, params.getParamMap(), getApiContext(),
          getResponseClass(), responseListener, errorListener);
    } else {
      DataFuture<T> dataFuture = new DataFuture<>();
      dataFuture.setLogEnable(isLogEnable);
      dataFuture.setIsNeedToastError(needToastError);
      request = new GsonRequest<T>(mMethod, requestUrl, params.getParamMap(), getApiContext(),
          getResponseClass(), dataFuture);
    }
