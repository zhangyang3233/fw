...
public class SlidingHelper {
  ...
  public static void onWindowFocusChanged(SlideActivity slideActivity, boolean hasFocus) {
    ...
    if (hasFocus) {
      if (isActivityFinished(slideActivity)) {
        return; // 如果该activity已经调用finish则前一个界面不需要重新执行动画
      }
  ...
  private static boolean isActivityFinished(SlideActivity slideActivity) {
    SlidingPager curPager = findPagerByActivity(slideActivity);
    if (curPager == null || curPager.getPageTransaction().isFinished()) {
      return true;
    }
    return false;
  }
  ...
  private static void setActivityFinished(SlideActivity slideActivity) {
    SlidingPager curPage = findPagerByActivity(slideActivity);
    if (curPage != null) {
      curPage.getPageTransaction().setIsFinished(true);
    }
  }
  ...
  private static synchronized void startEnterExitAnim(SlidingPager pager) {
  ...
  private static synchronized void startCloseEnterAnim(SlidingPager pager) {
    if (pager == null || pager.getSlidingLayout() == null) {
      return;
    }
  ...
  public static void setSlidingListener(SlideActivity activity, final SlidingListener listener) {
    ...
    if (pager.getSlidingLayout() == null) {
      return;
    }
