package com.zycoder.uicomp.stickyheader.listview;
...
public class StickyHeaderListView extends ListView {
  ...
  public interface OnHeaderClickListener {
    public void onHeaderClick(StickyHeaderListView l, View header,
  ...
  private InternalAdapterWrapper mAdapter;
  ...
  // TODO added by zycoder(xjs)
  private boolean mEnableStickyHeaderDescendantFocusability = false;

  public void setEnableStickyHeaderDescendantFocusability(boolean enable) {
    mEnableStickyHeaderDescendantFocusability = enable;
  }

  private InternalAdapterWrapper.OnHeaderClickListener mAdapterHeaderClickListener =
      new InternalAdapterWrapper.OnHeaderClickListener() {

        @Override
        public void onHeaderClick(View header, int itemPosition, long headerId) {
          if (mOnHeaderClickListener != null) {
            mOnHeaderClickListener.onHeaderClick(
                StickyHeaderListView.this, header, itemPosition,
                headerId, false);
          }
        }
      };
  ...
  private InternalAdapterWrapper wrapAdapter(ListAdapter adapter) {
    InternalAdapterWrapper wrapper;
    if (adapter instanceof SectionIndexer) {
      wrapper = new SectionIndexerAdapterWrapper(getContext(),
          (StickyHeaderListAdapter) adapter);
    } else {
      wrapper = new InternalAdapterWrapper(getContext(),
          (StickyHeaderListAdapter) adapter);
    }
  ...
  public StickyHeaderListAdapter getWrappedAdapter() {
      ...
      for (int i = 0; i < childCount; i++) {
        ...
        if (viewToWatch == null
            || (!viewToWatchIsFooter && !(viewToWatch instanceof WrapperView && ((WrapperView) viewToWatch)
                .hasHeader()))
            || ((childIsFooter || (child instanceof WrapperView && ((WrapperView) child)
    ...
    if (mHeaderBeingPressed) {
      if (Math.abs(ev.getY() - mHeaderDownY) < mViewConfig
        ...
        if (action == MotionEvent.ACTION_UP
          ...
          // TODO added by zycoder(xjs)
          if (mEnableStickyHeaderDescendantFocusability && mHeader instanceof ViewGroup) {
            ViewGroup group = (ViewGroup) mHeader;
            for (int index = 0; index < group.getChildCount(); ++index) {
              View child = group.getChildAt(index);
              if (child.isClickable()) {
                RectF rect =
                    new RectF(child.getLeft(), child.getTop(), child.getRight(), child.getBottom());
