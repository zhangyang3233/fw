...
public class ViewDragHelper {
  ...
  public static abstract class Callback {
     ...
     * It should do so using {@link #settleCapturedViewAt(int, int)} or
     * {@link #flingCapturedView(int, int, int, int)}. If the Callback invokes one of these methods,
     * the ViewDragHelper will enter {@link #STATE_SETTLING} and the view capture will not fully end
     * until it comes to a complete stop. If neither of these methods is invoked before
     * <code>onViewReleased</code> returns, the view will stop in place and the ViewDragHelper will
     * return to {@link #STATE_IDLE}.
     * </p>
     *
     ...
     * was preliminarily rejected before beginning, but after {@link #onEdgeTouched(int, int)} was
     * called. This method should return true to lock this edge or false to leave it
     ...
     * already captured; this indicates that a new pointer is trying to take control of the view.
     * </p>
     *
     * <p>
     * If this method returns true, a call to {@link #onViewCaptured(View, int)} will follow if the
   ...
   * @param view
   */
  void setCapturedView(View view) {
    mCapturedView = view;
  }
   ...
   * If this method returns true, the caller should invoke {@link #continueSettling(boolean)} on
   * each subsequent frame to continue the motion until it returns false. If this method
   * returns false there is no further work to do to complete the movement.
   *
   * <p>
   * This operation does not count as a capture event, though {@link #getCapturedView()} will still
   * report the sliding view while the slide is in progress.
   * </p>
   *
   ...
   *          Set this to true if you are calling this method from {@link View#computeScroll()} or
   *          similar methods
   ...
   * is the only time it is valid to call {@link #settleCapturedViewAt(int, int)} or
   ...
   * {@link #shouldInterceptTouchEvent(MotionEvent)} or {@link #processTouchEvent(MotionEvent)}. If
   * one of these methods has not been called for all relevant MotionEvents to track, the
   ...
   *
   * <p>
   * This depends on internal state populated by {@link #shouldInterceptTouchEvent(MotionEvent)} or
   * {@link #processTouchEvent(MotionEvent)}. You should only rely on the results of this method
   * after all currently available touch data has been provided to one of these two methods.
   * </p>
   *
   ...
   *          {@link #EDGE_TOP}, {@link #EDGE_RIGHT}, {@link #EDGE_BOTTOM} and {@link #EDGE_ALL}
