package com.zycoder.uicomp.multicolumn;
 ...
 *
 * <p>
 * See the <a href="{@docRoot}resources/tutorials/views/hello-listview.html">List View tutorial</a>.
 * </p>
 *
...
public class InternalListView extends InternalAbsListView {
   ...
   * <p>
   * NOTE: Call this before calling setAdapter. This is so ListView can wrap the supplied cursor
   * with one that will also account for header and footer views.
   *
  ...
  public boolean removeHeaderView(View v) {
    if (mHeaderViewInfos.size() > 0) {
      ...
      if (((InternalHeaderViewListAdapter) mAdapter).removeHeader(v)) {
  ...
  public boolean removeFooterView(View v) {
    if (mFooterViewInfos.size() > 0) {
      boolean result = false;
      if (((InternalHeaderViewListAdapter) mAdapter).removeFooter(v)) {
        mDataSetObserver.onChanged();
        result = true;
      }
  ...
  public void setAdapter(ListAdapter adapter) {
    ...
    if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
      mAdapter = new InternalHeaderViewListAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
    } else {
      mAdapter = adapter;
    }
  ...
  private void measureScrapChild(View child, int position, int widthMeasureSpec) {
    ...
    int childHeightSpec;
    if (lpHeight > 0) {
      childHeightSpec = View.MeasureSpec.makeMeasureSpec(lpHeight, View.MeasureSpec
          .EXACTLY);
    } else {
  ...
  final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition,
    ...
    final InternalAbsListView.RecycleBin recycleBin = mRecycler;
  ...
  private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft,
    ...
    InternalAbsListView.LayoutParams p = (InternalAbsListView.LayoutParams) child.getLayoutParams();
    if (p == null) {
      p = new InternalAbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
          ViewGroup.LayoutParams.WRAP_CONTENT, 0);
    }
    p.viewType = mAdapter.getItemViewType(position);

    if ((recycled && !p.forceAdd) || (p.recycledHeaderFooter &&
        p.viewType == InternalAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER)) {
      attachViewToParent(child, flowDown ? -1 : 0, p);
    } else {
      p.forceAdd = false;
      if (p.viewType == InternalAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER) {
    ...
    final int h = child.getMeasuredHeight() + mVerticalSpacing;
  ...
  private void scrollListItemsBy(int amount) {
    ...
    if (amount < 0) {
      ...
      while (first.getBottom() < listTop) {
        InternalAbsListView.LayoutParams layoutParams = (LayoutParams) first.getLayoutParams();
      ...
      while (last.getTop() > listBottom) {
        InternalAbsListView.LayoutParams layoutParams = (LayoutParams) last.getLayoutParams();
        if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
   ...
   *         or <code>null</code> if the choice mode is set to {@link #CHOICE_MODE_NONE}.
   */
