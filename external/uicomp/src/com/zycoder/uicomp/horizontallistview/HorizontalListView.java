...
public class HorizontalListView extends AbsHListView {
  ...
  private ArrayList<FixedViewInfo> mHeaderViewInfos =
      new ArrayList<HorizontalListView.FixedViewInfo>();
  ...
  public HorizontalListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    TypedArray a = context.obtainStyledAttributes(attrs,
        R.styleable.HorizontalListView, defStyle, 0);

    CharSequence[] entries = a
        .getTextArray(R.styleable.HorizontalListView_android_entries);
    if (entries != null) {
    ...
    final Drawable osHeader = a
        .getDrawable(R.styleable.HorizontalListView_hlvOverScrollHeader);
    if (osHeader != null) {
      setOverscrollHeader(osHeader);
    }

    final Drawable osFooter = a
        .getDrawable(R.styleable.HorizontalListView_hlvOverScrollFooter);
    if (osFooter != null) {
    ...
    mHeaderDividersEnabled = a.getBoolean(
        R.styleable.HorizontalListView_hlvHeaderDividersEnabled, true);
    mFooterDividersEnabled = a.getBoolean(
        R.styleable.HorizontalListView_hlvFooterDividersEnabled, true);
    mMeasureWithChild = a.getInteger(
        R.styleable.HorizontalListView_measureWithChild, -1);
   ...
   * adapter might not be the same adapter passed to {@link #setAdapter(ListAdapter)} but might be a
   ...
   * 
   * The adapter passed to this method may be wrapped by a {@link WrapperListAdapter}, depending on
   * the ListView features currently
  ...
  final int measureWidthOfChildren(int heightMeasureSpec, int startPosition,
    ...
    Log.i(LOG_TAG, "measureWidthOfChildren, from " + startPosition + " to "
        + endPosition);
   ...
   *          The position of the next selection, or {@link #INVALID_POSITION} if there is no next
   *          selectable
   *          position
  ...
  static private class ArrowScrollFocusResult {
    ...
    /**
     * How {@link HorizontalListView.sephiroth.android.library.widget.HListView#arrowScrollFocused}
     * returns its values.
     */
   ...
   * focus, return the selection delta and amount to scroll via an {@link ArrowScrollFocusResult},
