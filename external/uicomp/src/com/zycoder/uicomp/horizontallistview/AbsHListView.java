package com.zycoder.uicomp.horizontallistview;
...
import com.zycoder.uicomp.horizontallistview.util.ViewHelperFactory;
import com.zycoder.uicomp.horizontallistview.util.ViewHelperFactory.ViewHelper;
import com.zycoder.uicomp.horizontallistview.util.v11.MultiChoiceModeListener;
import com.zycoder.uicomp.horizontallistview.util.v11.MultiChoiceModeWrapper;
...
public abstract class AbsHListView extends AdapterView<ListAdapter> implements
  ...
  public AbsHListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    boolean stackFromRight = a.getBoolean(
        R.styleable.AbsHListView_stackFromRight, false);
    ...
    int transcriptMode = a.getInt(
        R.styleable.AbsHListView_hlvTranscriptMode,
        TRANSCRIPT_MODE_DISABLED);
    setTranscriptMode(transcriptMode);

    int color = a.getColor(R.styleable.AbsHListView_android_cacheColorHint,
        0);
    ...
    setChoiceMode(a.getInt(R.styleable.AbsHListView_android_choiceMode,
    ...
    if (mode != OVER_SCROLL_NEVER) {
      if (mEdgeGlowTop == null) {
        ...
        mEdgeGlowTop = new EdgeEffect(context,
            com.zycoder.uicomp.horizontallistview.EdgeEffect.DIRECTION_HORIZONTAL);
   ...
   *         get(int position) where position is a position in the list, or <code>null</code> if the
   *         choice mode is set to {@link #CHOICE_MODE_NONE}.
   */
   ...
   * the choice mode has been set to {@link #CHOICE_MODE_SINGLE} or {@link #CHOICE_MODE_MULTIPLE}.
   * 
   * @param position
   ...
   * to be in a chosen state. By setting the choiceMode to {@link #CHOICE_MODE_MULTIPLE}, the list
   * allows any number of items to be
   * chosen.
   * 
   * @param choiceMode
  ...
  boolean performLongPress(final View child, final int longPressPosition,
    ...
    if (android.os.Build.VERSION.SDK_INT >= 11) {
      if (mChoiceMode == ListView.CHOICE_MODE_MULTIPLE_MODAL) {
        if (mChoiceActionMode == null
            && (mChoiceActionMode =
                startActionMode((MultiChoiceModeWrapper) mMultiChoiceModeCallback)) != null) {
   ...
   * @param y
   *          Y in local coordinate
   * @return The colId of the item which contains the specified point, or {@link #INVALID_COL_ID} if
  ...
  protected float getHorizontalScrollFactor() {
    if (mHorizontalScrollFactor == 0) {
      ...
      if (!getContext().getTheme().resolveAttribute(
          R.attr.sephiroth_listPreferredItemWidth, outValue, true)) {
        throw new IllegalStateException(
   ...
   * {@link it.sephiroth.android.library.widget.AbsHListView.OnScrollListener} , if any. The state
   * change is fired only if the specified state is
   ...
   * will get as close as possible. The scroll will take <code>duration</code> milliseconds to
   * complete.
   * 
   * @param position
  ...
  protected void handleDataChanged() {
    ...
    if (android.os.Build.VERSION.SDK_INT >= 16
        && mChoiceMode != ListView.CHOICE_MODE_NONE && mAdapter != null
        && mAdapter.hasStableIds()) {
