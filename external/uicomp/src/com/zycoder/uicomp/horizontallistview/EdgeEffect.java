 ...
 * 
 * <p>
 * EdgeEffect is stateful. Custom widgets using EdgeEffect should create an instance for each edge
 * that should show the effect, feed it input data using the methods {@link #onAbsorb(int)},
 * {@link #onPull(float)}, and {@link #onRelease()}, and draw the effect using {@link #draw(Canvas)}
 * in the widget's overridden {@link android.view.View#draw(Canvas)} method. If
 * {@link #isFinished()} returns false after drawing, the edge effect's animation is not yet
 * complete and the widget should schedule another drawing pass to continue the animation.
 * </p>
 * 
 * <p>
 * When drawing, widgets should draw their main content and child views first, usually by invoking
 * <code>super.draw(canvas)</code> from an overridden <code>draw</code> method. (This will invoke
 * onDraw and dispatch drawing to child views as needed.) The edge effect may then be drawn on top
 * of the view's content using the {@link #draw(Canvas)} method.
 * </p>
 */
public class EdgeEffect {
   ...
   * associated animation. The host view should always {@link android.view.View#invalidate()} after
   * this and draw the results
   * accordingly.
   * 
   * @param deltaDistance
   ...
   * 
   * <p>
   * When using a {@link android.widget.Scroller} or {@link android.widget.OverScroller}, the method
   * <code>getCurrVelocity</code> will provide a reasonable approximation to use here.
   * </p>
   * 
   * @param velocity
