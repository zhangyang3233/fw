 ...
 */
public class SettingFragment extends BaseLoadFragment implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.setting_help:
                BrowserActivity.launch(getActivity(), Constants.Server.API_PREFIX + "/page/html?id=9", "帮助中心");
                break;
            case R.id.setting_terms:
                BrowserActivity.launch(getActivity(), Constants.Server.API_PREFIX + "/page/html?id=6", "法律条款");
                break;
            case R.id.setting_about_us:
                BrowserActivity.launch(getActivity(), Constants.Server.API_PREFIX + "/page/html?id=1", "关于我们");
                break;
            case R.id.logout:
                showLogoutDialog();
                break;
    ...
    private void showLogoutDialog() {
        ...
        dialog.setRight(getString(R.string.dialog_ok), new CommonTwoBtnDialogFragment.OnClickListener() {
            @Override
            public void onClick(Dialog dialog) {
                dialog.dismiss();
