...
public class PaySureFragment extends BaseLoadFragment implements OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.ok:
                showLoadingView();
                if (payType == ALI_APP) {
                    payZFB((int) (Float.valueOf(price) * 100f));
                } else {
                    payWX((int) (Float.valueOf(price) * 100f));
                }
                break;
    ...
    private void payWX(int priceInt) {
        ...
        // //商户自定义订单号
        // payParam.billNum = DateFormat.format("yyyyMMddHHmmss", System.currentTimeMillis())+orderId;
        // Map<String, String> optional = new HashMap<String, String>();
        //// optional.put("order_id",orderId);
        //// optional.put("user_id", AccountManager.getInstance().getUser().getUser_id());
